...
一个git管理的项目，都会有一个默认的分支（分支后面讲解），可以理解为档案管理员，它负责帮你记录各个版本，并负责帮你切换版本。  
git里面有几个概念，前台，缓存区，本地仓库，远程仓库：  
【前台】：就是我们当前能看到的文件夹内部  
【缓存区】：当我们在前台增删改文件后，我们不能直接将这些改动提交到本地仓库，因为你需要告诉档案管理员（master或者main）你要提交哪些改动，因为并不是所有改动都需要当前提交，比如程序员A今天完成了算法1的编写(algorithm1.py)，算法2(algorithm2.py)的编写完成到一半，这时候老板说先上传算法1给测试的同时区测试，那A就可以先提交algorithm1.py，然后继续写算法2。  
【本地仓库】:你向远程服务器提交的时候，始终是提交的这个本地仓库的文件（当前分支），档案管理员master（或者main）工作就是将缓存区的东西搬运到本地仓库，方便你后面向远程提交。  
...
```
git commit -m "finished algorithm1.py"  告诉当前档案管理员将缓存区的东西提交到本地仓库，并让他记录一下这一次操作做了什么，方便以后查看
```
...
# =============== 多人协作篇 =============== 
### ********************************** git分支管理及多人协作 *****************************
git中很重要的一个点就是分支的概念，分支就相当于一个档案管理员，正是有了分支的概念才有了多人协作的可能。不同分支之间不会影响，相当于每个管理员各自有一个自己的档案本。用户可以在不同的分支之间切换，可以将一个分支中的某个版本合并到另一个分支的另一个版本（可能设计到冲突需要处理，也可能没有，只有两个分支的版本都修改了同一个文件才会有冲突）。  
【1】一般情况下，一个个人项目会有一个master分支负责对外发布（最重要的是稳定性），这个分支上不做coding操作，只是适时把别的分支提交的东西合并进本分支，然后适当时间发布一次（可以打上标签，比如v1.0,v2.0等）。同时项目还会有一个develop分支，这个分支上可以编写代码，适时将代码提交给master分支。最后就是当master分支运行着的时候突然发现有问题，这时候会从master分支新建一个临时的fixbug分支，在其上修复bug，然后提交给master分支，待合并后就会删除这个fixbug分支。改过程图如下图所示：
<img src=img/fig11.png width="600" /><br/>
图中灰色线表示只是名义上的版本迭代关系，实际上不是沿着这个路径迭代的。  
【2】当有多个人协作开发时，一般时大型项目，这时候就更规范，一般devolop分支也不用来coding，而是用来作为测试版本，负责合并所有开发者提交的代码，然后适当提交给master去发布。具体结构图如下图所示：
<img src=img/fig12.png width="600" /><br/>
每个程序员会先把项目的develop分支clone下来，这时候本地默认也是develop分支（这个分支必须一直和远程最新的develop分支一致，否则向远程develop分支提交的时候）
怎么添加新的开发人员？？
### *************************************** git常用配置方法 ***************************************
